...
**
** Purpose:
** Coverage Unit Test cases for the Storage Demo Application
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the Storage Demo Application.
**
...
void Test_BUFFER_MGR_APP_Main(void)
{
    ...
    /*
     * BUFFER_MGR_APP_Main does not return a value,
     ...
     * The first is for BUFFER_MGR_APP_Init().  As this is in the same
     ...
     * the BUFFER_MGR_APP_Init() failure path was taken.
     */
    BUFFER_MGR_APP_Main();
     ...
     */
    UtAssert_UINT32_EQ(BUFFER_MGR_APP_Data.RunStatus, CFE_ES_RunStatus_APP_ERROR);
    ...
    UT_CHECKEVENT_SETUP(&EventTest, BUFFER_MGR_APP_PIPE_ERR_EID, "STO DEMO APP: SB Pipe Read Error, App Will Exit");
...
void Test_BUFFER_MGR_APP_Init(void)
{
    /*
     * Test Case For:
    ...
    UtAssert_INT32_EQ(BUFFER_MGR_APP_Init(), CFE_SUCCESS);
     ...
     */
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UtAssert_INT32_EQ(BUFFER_MGR_APP_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 1);

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UtAssert_INT32_EQ(BUFFER_MGR_APP_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UtAssert_INT32_EQ(BUFFER_MGR_APP_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
...
{
    ADD_TEST(BUFFER_MGR_APP_Main);
    ADD_TEST(BUFFER_MGR_APP_Init);
}
