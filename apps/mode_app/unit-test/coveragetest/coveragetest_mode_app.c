...
**
** Purpose:
** Coverage Unit Test cases for the Mode Application
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the Mode application.
**
...
void Test_MODE_APP_Main(void)
{
    ...
    /*
     * MODE_APP_Main does not return a value,
     ...
     * The first is for MODE_APP_Init().  As this is in the same
     ...
     * the MODE_APP_Init() failure path was taken.
     */
    MODE_APP_Main();
     ...
     */
    UtAssert_UINT32_EQ(MODE_APP_Data.RunStatus, CFE_ES_RunStatus_APP_ERROR);
    ...
    UT_CHECKEVENT_SETUP(&EventTest, MODE_APP_PIPE_ERR_EID, "MODE APP: SB Pipe Read Error, App Will Exit");
...
void Test_MODE_APP_Init(void)
{
    ...
    UtAssert_INT32_EQ(MODE_APP_Init(), CFE_SUCCESS);
     ...
     */
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UtAssert_INT32_EQ(MODE_APP_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 1);

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UtAssert_INT32_EQ(MODE_APP_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UtAssert_INT32_EQ(MODE_APP_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
...
{
    ADD_TEST(MODE_APP_Main);
    ADD_TEST(MODE_APP_Init);
}
