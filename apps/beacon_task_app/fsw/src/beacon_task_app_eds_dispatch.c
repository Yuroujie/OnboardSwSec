...
#include "beacon_task_app_eds_dispatcher.h"
#include "beacon_task_app_eds_dictionary.h"

/*
 * Define a lookup table for SAMPLE app command codes
 */
/* clang-format off */
static const EdsDispatchTable_EdsComponent_BEACON_TASK_APP_Application_CFE_SB_Telecommand_t SAMPLE_TC_DISPATCH_TABLE = 
{
    .CMD =
    {
        .NoopCmd_indication          = BEACON_TASK_APP_NoopCmd,
        .ResetCountersCmd_indication = BEACON_TASK_APP_ResetCountersCmd,
        .ProcessCmd_indication       = BEACON_TASK_APP_ProcessCmd,
        .DisplayParamCmd_indication  = BEACON_TASK_APP_DisplayParamCmd
    },
    .SEND_HK =
    {
        .indication = BEACON_TASK_APP_SendHkCmd
    }
};
/* clang-format on */
...
void BEACON_TASK_APP_TaskPipe(const CFE_SB_Buffer_t *SBBufPtr)
{
    ...
    Status = EdsDispatch_EdsComponent_BEACON_TASK_APP_Application_Telecommand(SBBufPtr, &SAMPLE_TC_DISPATCH_TABLE);
    ...
    if (Status != CFE_SUCCESS)
    {
        ...
        {
            CFE_EVS_SendEvent(BEACON_TASK_APP_CC_ERR_EID, CFE_EVS_EventType_ERROR,
