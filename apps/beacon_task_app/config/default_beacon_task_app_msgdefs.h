 ...
 */
#ifndef DEFAULT_BEACON_TASK_APP_MSGDEFS_H
#define DEFAULT_BEACON_TASK_APP_MSGDEFS_H

#include "common_types.h"
#include "beacon_task_app_fcncodes.h"

typedef struct BEACON_TASK_APP_DisplayParam_Payload
{
    uint32 ValU32;                                    /**< 32 bit unsigned integer value */
    int16  ValI16;                                    /**< 16 bit signed integer value */
    char   ValStr[BEACON_TASK_APP_MISSION_STRING_VAL_LEN]; /**< An example string */
} BEACON_TASK_APP_DisplayParam_Payload_t;

typedef struct BEACON_TASK_APP_SimTraffic_Payload
{
...
} BEACON_TASK_APP_SimTraffic_Payload_t;

typedef struct BEACON_TASK_APP_RunBurst_Payload
{
    uint16 Count;       /**< Number of simulated fast commands to process */
    uint16 SpacingMsec; /**< Inter-command spacing in milliseconds */
} BEACON_TASK_APP_RunBurst_Payload_t;
...
typedef struct BEACON_TASK_APP_HkTlm_Payload
{
...
} BEACON_TASK_APP_HkTlm_Payload_t;
