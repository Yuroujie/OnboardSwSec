 ...
 */
#ifndef DEFAULT_AUTONOMY_GUARD_APP_MSGDEFS_H
#define DEFAULT_AUTONOMY_GUARD_APP_MSGDEFS_H

#include "common_types.h"
#include "autonomy_guard_app_fcncodes.h"

typedef struct AUTONOMY_GUARD_APP_DisplayParam_Payload
{
    uint32 ValU32;                                    /**< 32 bit unsigned integer value */
    int16  ValI16;                                    /**< 16 bit signed integer value */
    char   ValStr[AUTONOMY_GUARD_APP_MISSION_STRING_VAL_LEN]; /**< An example string */
} AUTONOMY_GUARD_APP_DisplayParam_Payload_t;

typedef struct AUTONOMY_GUARD_APP_SetApState_Payload
{
    uint16 ApNumber;   /**< Simulated limit-checker actionpoint number */
    uint8  NewApState; /**< 1=active, 2=passive, 3=disabled */
    uint8  Padding;
} AUTONOMY_GUARD_APP_SetApState_Payload_t;

typedef struct AUTONOMY_GUARD_APP_DisableCheck_Payload
{
    uint32 EntryId; /**< Simulated checksum or monitor entry identifier */
} AUTONOMY_GUARD_APP_DisableCheck_Payload_t;
...
typedef struct AUTONOMY_GUARD_APP_HkTlm_Payload
{
...
} AUTONOMY_GUARD_APP_HkTlm_Payload_t;
