...
#include "storage_audit_app_eds_dispatcher.h"
#include "storage_audit_app_eds_dictionary.h"

/*
 * Define a lookup table for SAMPLE app command codes
 */
/* clang-format off */
static const EdsDispatchTable_EdsComponent_STORAGE_AUDIT_APP_Application_CFE_SB_Telecommand_t SAMPLE_TC_DISPATCH_TABLE = 
{
    .CMD =
    {
        .NoopCmd_indication          = STORAGE_AUDIT_APP_NoopCmd,
        .ResetCountersCmd_indication = STORAGE_AUDIT_APP_ResetCountersCmd,
        .ProcessCmd_indication       = STORAGE_AUDIT_APP_ProcessCmd,
        .DisplayParamCmd_indication  = STORAGE_AUDIT_APP_DisplayParamCmd
    },
    .SEND_HK =
    {
        .indication = STORAGE_AUDIT_APP_SendHkCmd
    }
};
/* clang-format on */
...
void STORAGE_AUDIT_APP_TaskPipe(const CFE_SB_Buffer_t *SBBufPtr)
{
    ...
    Status = EdsDispatch_EdsComponent_STORAGE_AUDIT_APP_Application_Telecommand(SBBufPtr, &SAMPLE_TC_DISPATCH_TABLE);
    ...
    if (Status != CFE_SUCCESS)
    {
        ...
        {
            CFE_EVS_SendEvent(STORAGE_AUDIT_APP_CC_ERR_EID, CFE_EVS_EventType_ERROR,
